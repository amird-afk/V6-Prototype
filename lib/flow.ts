...
// Partial (12 steps — no plan-selection, no scholarship-agreement)
const partialSteps: Record<string, number> = {
  '/pql': 1, '/eligibility': 2, '/signup-pql': 3, '/program-selection': 4,
  '/fafsa': 5, '/stripe-poe': 6, '/diploma-upload': 7, '/q-verification': 8,
  '/tuition-package': 9, '/enrollment-documents': 10, '/pay-flow': 11, '/finished': 12,
};
...
export function getStepInfo(pathname: string, track: Track | null) {
  ...
  if (track === 'partial') return { step: partialSteps[base] ?? 0, total: 12, trackLabel: 'Partial scholarship' };
